...
function checkAnswer(){
  ...
  if (`${COUNTRYDATA[questionNumber-1].correctAnswer}` !== (undefined || null)){
...
function onSubmit(){
  $('#qform1').on('submit', function(event){
    event.preventDefault();
    if (questionNumber <= 5){
      $('input:checked') ? 
        checkAnswer() :
...
function nextQuestion() {
  questionNumber++;
  let site = null;
  $('.quizTitle').css('background', 'rgba(63, 99, 153, 0.5)');
  $('ul').css('background', 'rgba(63, 99, 153, 0.5)');
  $('.currentOption').css('color', 'black');
  $('.response').css('visibility', 'hidden');
  $('.questionForm').css('display','block');
  $('.questionForm').css('visibility', 'visible');
  ...
  if  ((questionNumber-1) < sizeOfDataObject) {
    $('.insert').html(COUNTRYDATA[questionNumber-1].questions);
